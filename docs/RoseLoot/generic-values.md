...
| `input-item` | Requires the input item to match | ENTITY_DROP_ITEM, PIGLIN_BARTER, HARVEST | [Material](https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/Material.html) or [Tags](https://minecraft.wiki/w/Tag#List_of_tags), requires 1 or more | `input-item:pink_wool` |
...
| `placeholder` | Parsed placeholder must match given value with given operation | ALL | [PlaceholderAPI](https://www.spigotmc.org/resources/placeholderapi.6245/) is supported, [Context Placeholders](https://github.com/Rosewood-Development/RoseLoot/wiki/Context-Placeholders) are also supported.  Operation is required and one of the following: `!=`, `<=`, `>=`, `=`, `<`, `>`, `^` (contains) | `placeholder:%player_biome%^hills` |
